...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
     apt-get update
     apt-get install -y emacs tcsh gcc bzr git make pkg-config

     sudo -u vagrant mkdir -p /vagrant/go/src
     sudo -u vagrant cp -r /vagrant/{.cshrc,.emacs,.emacs.d,.bashrc} .
