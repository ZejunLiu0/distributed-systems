...
func postToServer(msg *Message, addr string, errMsg string) map[string]interface{} {
	...
	if res.StatusCode != 200 {
		...
		os.Stderr.WriteString(fmt.Sprint(res.StatusCode) + ": Invalid root name\n")

		if errMsg != "" {
			os.Stderr.WriteString("Invalid root name\n")
		}
		return nil
	}
...
func CmdAnchor(privateKeyF string, rootname string) string {
	fmt.Println("ServerAddress in cmdanchor:", ServerAddress)
	// private := readPrivateKey(privateKeyF)
	randID := make([]byte, 32)
	rand.Read(randID)
	...
	data := postToServer(&msg, ServerAddress, "")
	PrintAssert(data != nil, "data is nil\n")
...
func CmdClaim(privateKeyF string, anchorRootname string, KVpairs []string) {
	...
	currtime := time.Now()
	fmt.Println("\nTime travel checkpoint:", currtime)
	claimChunk := Message{
		Version: 1,
		Type:    "claim",
		Refsig:  rootSig,
...
func CmdLastclaim(rootname string) string {
	msg := Message{
		Version: 1,
		Type:    "lastclaim",
		Name:    rootname,
	...
	// fmt.Println("CmdLastclaim:", lastSig)
	return lastSig
}
